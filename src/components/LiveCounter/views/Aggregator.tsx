...
interface AggregatorComponentProps {
  ...
  setMainTabBadgeCounts: Dispatch<SetStateAction<{
    Tables: number;
    Delivery: number;
    Pickup: number;
    Aggregators: number;
...
export default function AggregatorComponent({searchKeyword, setMainTabBadgeCounts}: AggregatorComponentProps) {
  const { brandId } = useAuth()
  const bottomRef = useRef<HTMLDivElement>(null)
  const [selectedItem, setSelectedItem] = useState<OrderListItem | undefined>(undefined);
  
  useEffect(()=>{
    setFilters((prevFilters)=>updateFilters(prevFilters, "search", searchKeyword))
  },[searchKeyword])
  ...
  useEffect(()=>{
    if (data)
      setMainTabBadgeCounts((prevState)=>updateFilters(prevState, "Aggregators", data.pages[0].total || 0))
  },[data])
  ...
  const options = [
    { value: LiveCounterOrderSortingOption.ASCENDING_STATUS, label: "Order No. Ascending" },
    { value: LiveCounterOrderSortingOption.DESCENDING_STATUS, label: "Order No. Descending" },
    { value: LiveCounterOrderSortingOption.ASCENDING_TIME_STATUS, label: "Order Time. Ascending" },
    { value: LiveCounterOrderSortingOption.DESCENDING_TIME_STATUS, label: "Order Time. Descending" },
  ]
  ...
  const handleSortChange = (option: {
    ...
    if (option.value === LiveCounterOrderSortingOption.ASCENDING_STATUS) {
      setFilters((prevFilters) => updateFilters(prevFilters, "sort_by", "ordernumber"))
      setFilters((prevFilters) => updateFilters(prevFilters, "sort_order", "asc"))
    } else if (option.value === LiveCounterOrderSortingOption.DESCENDING_STATUS) {
      setFilters((prevFilters) => updateFilters(prevFilters, "sort_by", "ordernumber"))
      setFilters((prevFilters) => updateFilters(prevFilters, "sort_order", "desc"))
    } else if (option.value === LiveCounterOrderSortingOption.ASCENDING_TIME_STATUS) {
      setFilters((prevFilters) => updateFilters(prevFilters, "sort_by", "date"))
      setFilters((prevFilters) => updateFilters(prevFilters, "sort_order", "asc"))
    } else if (option.value === LiveCounterOrderSortingOption.DESCENDING_TIME_STATUS) {
      setFilters((prevFilters) => updateFilters(prevFilters, "sort_by", "date"))
      setFilters((prevFilters) => updateFilters(prevFilters, "sort_order", "desc"))
    }
  }
  ...
  const handleTabChange = ({tabIndex}: {tabIndex: number}) => {
    ...
    if (tabIndex === 0) {
      setFilters((prevFilters) => updateFilters(prevFilters, "order_status", "ordered,accepted,ready"))
    } else {
      setFilters((prevFilters) => updateFilters(prevFilters, "order_status", "closed,delivered"))
    }
  }
  ...
  return (
    ...
    {!isLoading && (
      <>
          ...
          <CartProvider>
            <PickupDetailDialog item={selectedItem} onClose={()=>{setSelectedItem(undefined)}}
              isAggregator={true} 
              onUpdate={()=>{
                refetch()
              }}   
